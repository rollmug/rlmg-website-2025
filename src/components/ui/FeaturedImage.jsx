...
export const FeaturedImage = ({ src, alt, width = 450 }) => {
    // width is the max width of the image, so it doesn't stretch
    return (
        <>
            <AnimatePresence>
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ ease: "easeIn", duration: .2 }} className="relative max-w-max">
                    ...
                    <motion.div initial={{ rotate: 6 }} whileInView={{ rotate: 0 }} transition={{ ease: "easeIn", duration: .5, delay: .5 }}>
                        <Image src={src} alt={alt} width={width} height={0} className="relative" />
                    </motion.div>
                </motion.div>
            </AnimatePresence>
        </>
